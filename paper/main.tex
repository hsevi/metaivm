...
\def\month{09}  % September (camera-ready publication; acceptance decision was 2026-08-18)
\def\year{2026}
...
In practice, this assumption often fails. IVMs behave inconsistently across datasets: a measure that identifies the best clustering on one dataset may prefer the worst clustering on another~\citep{simpson2026instance, jeon2025measuring}. On our benchmark, the strongest classical measure, Calinski-Harabasz, attains only $\rho = 0.60$ average per-dataset rank correlation with external agreement (AMI) and a selection regret of 0.217, meaning it selects clusterings that are, on average, 21.7\% worse than the best available. The density-based measure DBCV~\citep{moulavi2014dbcv} is no better under the same protocol, and the Silhouette Coefficient is worse still.
...
The key insight is that the evidence predictive of external agreement is context-dependent, varying with data geometry. On spherical clusters, dispersion ratio is the strongest signal; on manifolds, graph conductance dominates; on imbalanced data, cluster size statistics carry most of the information. No single fixed IVM can adapt its evaluation criterion (Appendix~\ref{app:theory} provides a constructive illustration of a specific CH failure mode on anisotropic clusters). A supervised model trained on diverse (dataset, partition, quality) triples learns which signals matter where. Ablations support that the main gains come from the formulation and feature space rather than a specific model: removing IVM features entirely barely changes performance (0.067 vs.\ 0.071 regret), and even linear regression on our features beats every classical IVM (0.088 vs.\ 0.217 for the best of them).
...
Closer to our per-partition formulation, two prior methods train learned surrogates that map internal validity measures and/or dataset meta-features to external agreement. AutoClust~\citep{poulakis2020autoclust} trains an MLP on a fixed vector of $10$ classical IVMs of a candidate clustering to predict Adjusted Rand Index, and uses it as the fitness function inside a Bayesian-optimization loop for hyperparameter tuning; algorithm selection is handled separately by k-nearest neighbors over the same IVM vector. PoAC~\citep{dasilva2024poac} trains a Random Forest on $37$ dataset-level meta-features~\citep{lorena2019complexity} plus two IVMs (Silhouette, Davies-Bouldin), fit on a meta-database whose training partitions are generated by adding Gaussian noise to ground-truth labels, and uses it as the fitness function in an evolutionary pipeline search.
...
First, feature space: MetaIVM's default representation is a $28$-dimensional \emph{partition-structural} vector (cluster-size statistics, entropy, imbalance, within/between-cluster variance, centroid separations, cluster densities, and $6$ graph-topology features such as conductance and normalized cut), whereas AutoClust uses $10$ IVMs of the candidate and PoAC uses $37$ dataset meta-features plus $2$ IVMs. Two controls probe these alternative feature spaces on our benchmark: a model learned on IVMs alone (Ridge on $3$ IVMs, regret $0.337$) and one on IVMs plus dataset descriptors (XGBoost, regret $0.143$) both land far above MetaIVM's $0.071$, while removing all IVM features from MetaIVM leaves its regret essentially unchanged ($0.067$; Table~\ref{tab:ablation}). Partition-structural features, not the combination or learning of IVMs, drive the improvement.
...
\begin{itemize}
    ...
    \item \textbf{MLP}: ReLU activations, Adam optimizer, standardized features. Grid search over hidden architectures $\{(256,128), (512,256), (256,128,64), (512,256,128)\}$ and learning rates $\{10^{-3}, 3 \cdot 10^{-3}\}$, each trained on the dataset-level train fold with arch/lr selected on the dataset-level validation fold. We disable the library's internal early stopping, whose row-level split would otherwise mix candidate partitions of the same dataset across the internal train and validation sets. Full details in Appendix~\ref{app:training}.
...
\paragraph{Label semantics and the choice of target.} Our benchmarks treat each dataset's classification labels as the reference partition for AMI, an operational choice: such labels give an unambiguous, reproducible ground truth and are the target used by most prior meta-learning work on clustering. We do not claim they define the uniquely correct clustering of a dataset. The ``natural'' grouping is often task-specific: customers, for instance, can be partitioned by demographics, purchase behavior, lifetime value, or risk profile, all valid and all different. \ours{} is a framework rather than a fixed policy about what makes a clustering good; the same infrastructure applies to hierarchical labels, application-specific tags, or user-defined similarity signals, provided a labeled meta-benchmark encodes them. It is therefore directly applicable when the deployment target matches the benchmark target (classification-label recovery on tabular, image, or text data), and users with a different target in mind, one that is multi-resolution or orthogonal to class labels, should retrain on labeled examples that reflect it. This dependence on the training notion of quality is inherent to any supervised surrogate for unsupervised evaluation and is shared with AutoClust~\citep{poulakis2020autoclust}, PoAC~\citep{dasilva2024poac}, and dataset-level meta-selection work.
...
\paragraph{Limitations.} Six caveats bound our claims. (1) \ours{} requires offline training on labeled datasets, though purely synthetic data suffices when real labels are unavailable. (2) It predicts agreement with a reference partition (AMI), one operationalization of quality; applications with domain-specific criteria may rank partitions differently. (3) Evidence on specialized domains such as genomics and time series remains limited. (4) The main benchmark omits Spectral Clustering on cost grounds ($O(n^3)$); adding it post-hoc raises regret by only 0.005, leaving our conclusions unchanged. (5) Accuracy degrades on high-noise, subspace, and extreme-imbalance data, which make up the 19\% of test cases with regret above 0.15. (6) The AutoClust-style baseline reproduces only four of the ten CVIs in the original method; the other six (S\_Dbw, CDbw, Tau, Ratkowsky-Lance, McClain-Rao, C-Index) lack maintained Python implementations, so their effect on its regret is unknown.
...
Choosing among candidate clusterings without labels is a problem that fixed internal validity measures solve only weakly. We showed that a learned per-partition surrogate does substantially better: on our benchmark of 223 datasets, \ours{} attains a selection regret of 0.071, a 67\% reduction over the strongest classical baseline (Calinski-Harabasz, 0.217), and its advantage over Silhouette, CH, and DB is significant at Wilcoxon $p < 10^{-12}$. Density-based measures (DBCV 0.219, S\_Dbw 0.334) match CH but do no better under the same protocol. These gains are consistent across agreement metrics and model families and carry over to graph community detection. Controlled ablations attribute them to the per-partition formulation and its feature space rather than to learning capacity or the presence of IVMs alone: swapping either component away removes most of the benefit. The principal limitation is distributional: accuracy is tied to the training benchmark, and transfer to domains far from it remains open, calling for broader benchmarks and features less specific to tabular data. All data, code, and a ready-to-use scikit-learn-style selection interface are available at \url{https://github.com/hsevi/metaivm}.

\paragraph{Ethics and reproducibility.}
We foresee no significant negative societal impact. The benchmark uses only publicly available datasets. All code, datasets, and experiment scripts are released as open-source. The benchmark is fully reproducible with fixed random seeds (0--4) for all splits.
